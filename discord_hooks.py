...
class Webhook(object):
	...
	def post(self):
		...
		resp = requests.post(self.url, data=self.json, headers=headers)
		...
		try:
			...
			if rate_limit_remaining <= 0:
				...
				print(time_to_sleep)		
				print(f'Rate limit encountered! Sleeping {time_to_sleep} seconds')
